...
const messages = {
  en: {
    ...
    Description_poem_type: "This is a %s1. Poem content: %s2",
...
<p>After generating a poem using AI Poem Generator, click the copy button to copy the poem and then paste it to any platform you want to send it to.</p>`
  ...
  fr: {
    ...
    Description_poem_type: "Il s'agit d'un %s1. Contenu du poème : %s2",
...
<p>Après avoir généré un poème à l'aide de Générateur de poème, cliquez sur le bouton Copier pour copier le poème, puis collez-le sur la plateforme à laquelle vous souhaitez l'envoyer.</p>`,
  ...
  de: {
    ...
    Description_poem_type: "Dies ist ein %s1. Gedichtinhalt: %s2",
...
<p>Nachdem Sie mit Gedichten Generator ein Gedicht erstellt haben, klicken Sie auf die Schaltfläche „Kopieren“, um das Gedicht zu kopieren und es dann auf einer beliebigen Plattform einzufügen, an die Sie es senden möchten.</p>`
  ...
  ru: {
    ...
    Description_poem_type: "Это %s1. Содержание стихотворения: %s2",
...
<p>После создания стихотворения с помощью Генератор Стихов нажмите кнопку «Копировать», чтобы скопировать стихотворение, а затем вставьте его на любую платформу, на которую вы хотите его отправить.</p>`
  ...
  it: {
    ...
    Description_poem_type: "Questo è un %s1. Contenuto della poesia: %s2",
...
<p>Dopo aver generato una poesia utilizzando Generatore di poesie, fai clic sul pulsante Copia per copiare la poesia e quindi incollarla su qualsiasi piattaforma a cui desideri inviarla.</p>`
  ...
  pt: {
    ...
    Description_poem_type: "Este é um %s1. Conteúdo do poema: %s2",
...
<p>Após gerar um poema usando o Gerador de Poemas, clique no botão copiar para copiar o poema e cole-o em qualquer plataforma para a qual deseja enviá-lo.</p>`
  ...
  es: {
    ...
    Description_poem_type: "Este es un %s1. Contenido del poema: %s2",
...
<p>Después de generar un poema usando el Generador de Poemas, haga clic en el botón copiar para copiar el poema y luego péguelo en cualquier plataforma a la que desee enviarlo.</p>`
  ...
  ja: {
    ...
    Description_poem_type: "これは %s1 です。 詩の内容: %s2",
...
<p>AI詩ジェネレーターを使用して詩を生成した後、コピーボタンをクリックして詩をコピーし、送信先のプラットフォームに貼り付けます。</p>`
  ...
  ko: {
    ...
    Description_poem_type: "이것은 %s1이다. 시 내용: %s2",
...
<p>AI 시 생성기를 사용하여 시를 생성한 후 복사 버튼을 클릭하여 시를 복사한 다음 보내고 싶은 플랫폼에 붙여넣습니다.</p>`
  ...
  th: {
    ...
    Description_poem_type: "นี่คือ %s1 เนื้อหาบทกวี: %s2",
...
<p>หลังจากสร้างบทกวีโดยใช้ ตัวสร้างบทกวี AI ให้คลิกปุ่มคัดลอกเพื่อคัดลอกบทกวี จากนั้นวางลงในแพลตฟอร์มใดๆ ที่คุณต้องการส่งไป</p>`
  ...
  id: {
    ...
    Description_poem_type: "Ini adalah %s1. Isi puisi: %s2",
...
`
  ...
  vi: {
    ...
    Description_poem_type: "Đây là %s1. Nội dung thơ: %s2",
...
`
  ...
  ar: {
    ...
    Description_poem_type: "هذه %s1. محتوى الشعر: %s2",
...
<p>بعد إنشاء قصيدة باستخدام مولد الشعر الآلي، انقر فوق زر النسخ لنسخ القصيدة ثم لصقها على أي منصة تريد إرسالها إليها.</p>`
  ...
  tr: {
    ...
    Description_poem_type: "Bu bir %s1. Şiir İçeriği: %s2",
